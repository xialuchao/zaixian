...
import logging
import os
from android.util import config_path

class LoggerHandler(logging.Logger): #TODO:此处用继承的方法可以解决输出行号不对的问题
    def __init__(self,
                 log_name="android_log",
                 filename=os.path.join(config_path.log_path,"log.txt"),
                 HandlerLevel="DEBUG",
                 StreamHandler="DEBUG"):
        ...
        if log_name:
            handler=logging.FileHandler(filename, encoding="utf8") #file文件输出
